/*********************************************************************/
/* FILE cvcads.c : program to convert the hexa CCITT test sequence   */
/*                 in ads56116 file format                           */
/*********************************************************************/
...
FILE *xin,*xout;
		...
		printf("\n Use: CVCADS fich.xxx ADSfich.xxx");
		...
		exit(1);
	     ...
	     /* print the ads file */
	     /***********************/
	     for(i=0,ligne=0; i <nin;i++){
	       if ( ligne < 7){
	       fprintf(xout," %04x",sqin[i]);
	       ligne++;
	       }
	       else {
	       fprintf(xout," %04x\n",sqin[i]);
