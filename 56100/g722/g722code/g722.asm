...
;; 	File		: g722.asm 				       	 	;
;;										;
;;	Date		: October 4th 1990, 14:26				;
;;										;
;;      Program for testing the encoder and decoder of G722. Including the QMF	;
;;	transmit and receive filters, ADC (56ADC16 Sigma-delta) input and the	;
;;	DAC (Burr-Brown PCM56) output.						;
;;										;
...
;;	(file MODCOD.TST).							;
;;										;
;;	The network is simulated by putting IS into IR in the program.		;
;;										;
;;	This version uses interrupts for the A/D and D/A connected to the ADS	;
;;	board but does not cope with network interrupt. 			;
;;		                                                                ;
...
set_ipl		equ $0C00			; enable ssi0 interrupt
...
set_pcc		equ $02ff			; set port C as SSI
;;
;;	SSI0 equates
;;	============
;;
...
;;
set_cra_0	equ $6000                       ; no prescaler
						; 16 bits
						; normal operation
;;
set_crb_0a	equ $0500			; no rx or tx enable yet
set_crb_0	equ $b500                       ; rx interrupt enable
						; normal mode
						; synchronous mode
						; active high frame sync.
						; single bit frame sync.
						; msb first
						; data out rising clock edge
						; data in falling clock edge
						; external clock
						; external frame sync.
						; SC00 & SC10 frame sync i/o
...
;;
	org	p:$0010
	jsr roe_rx_0			; receive rx_0 with execption
;;
	org	p:$0012
	jsr rok_rx_0			; recieve rx_0 ok
;;
;;
;;****************************************************************
;;
...
;;	===============
;;
		move 	#set_cra_0,b	; set mode cra in x0
		move 	b1,x:a_cra_0	; set mode in cra_0
...
;;
		...
		move 	x0,x:a_ipl	; write ipl
		andi 	#set_mr,mr	; unmask all interrupts
;;
;;
;;	Clear input_flag at 16 kHz
;;	==========================
;;
		clr 	b
		move 	b,x:flag_in	; set flag_in to 0
;;
;;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
;;
;;	==========
...
;;
;;	Wait for two inputs at 16 kHz
;;	=============================
;;
wait_2i		bftsth 	#$0002,x:flag_in	; test flag_in == 2
		bcc 	<wait_2i		; wait for 2 inputs at 16 kHz
;;
;;	Ok for 2 inputs at 16 kHz
;;	=========================
;;
;;	Read adc values
;;	===============
;;
		move 	x:adc_in_1,x0	; read input#1 of adc
		move 	x0,x:x1_in	; older sample
		move 	x:adc_in_0,x0	; read input#0 of adc
		move 	x0,x:x0_in	; newer sample
;;
;;
;;	Write dac values
;;	================
;;
		move 	x:xout1,x0	; read 1st output of qmfrx
		move 	x0,x:dac_out_1	; output #1 for dac
		move 	x:xout2,x0	; read 2nd output of qmfrx
		move 	x0,x:dac_out_2	; output #2 for dac
;;
;;	Set flag_in to 0 for following interrupt
;;	========================================
;;
		clr 	b		; accb = 0
		move	b,x:flag_in	; set flag _in to 0
;;
;;*******************************************************************
;;
...
;;
;;	Simulation of network transmission
;;	==================================
;;
		move x:is,a		; load computed code
		move a,x:ir		; save new ouput code
;;
;;	Call encoder procedure
;;	======================
;;
		jsr decoder		; call G722 encoder
;;
;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
init_q_rx move 	#dat_q_rx,r0		; adress of delay line
	clr 	a			; a to 0
...
;;	================================================================
;;
	move 	x:x1_in,b		; recent the previous one
	move 	x:x0_in,a		; read the more recent
...
;;	hsbdec); the outputs are xout1 and xout2 (the more recent)
;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
...
;;
	...
	asl 	b			; 
	move 	b,x:xout2		; the more recent
	move 	a,x:xout1		; the previous one
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;;	SSIO RX_0 interrupt recieve section
;;	===================================
;;
roe_rx_0 nop				; follows the noemal interrupt
rok_rx_0 move x0,x:sav_x0		; save register x0
	 move x:a_sr_0,x0		; clear execption
;;
;;	=====================================================
;;	If flag_in == 0, input adc_in_1 and output dac_out_1
;;	then set flag_in to 1
;;	=====================================================
;;
ok_in_1	bfset 	#$0001,x:flag_in  	; set flag_in to 0 if not set
	bcs 	ok_in_2			; was set to 1
	move 	x:a_txrx_0,x0		; read received word
	move 	x0,x:adc_in_1		; save 1st adc value
	move 	x:dac_out_1,x0		; read 1st dac value
	move 	x0,x:a_txrx_0		; save transmit word
	move 	x:sav_x0,x0		; restore x0
	rti
;;
ok_in_2 bfchg 	#$0003,x:flag_in  	; set flag_in to 2
	move 	x:a_txrx_0,x0		; read received word
	move 	x0,x:adc_in_0		; save 2nd adc value
	move 	x:dac_out_2,x0		; read 2nd dac value
	move 	x0,x:a_txrx_0		; save transmit word
	move 	x:sav_x0,x0		; restore x0
	rti
;;
